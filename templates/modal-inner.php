...
if ( ! empty( $post_data_id ) ) { ?>
    <div class="c-bfml-modal__inner <?= BFModalConfig::get_modal_inner_js_class(); ?>">
        <button class="c-bfml-modal__close u-b1 <?= BFModalConfig::get_modal_close_btn_js_class(); ?>"
                data-return-url="<?= ! empty( $args['return_url'] ) ? $args['return_url'] : ''; ?>">
            <span><?= __( 'Close', BFConstants::BFML_DOMAIN_NAME ); ?></span>
        ...
        <div class="c-bfml-modal__content">
            <div class="c-bfml-modal__row">
                <div class="c-bfml-text-edit">
                    <?= apply_filters( 'the_content', get_post_field( 'post_content', $post_data_id ) ) ?>
                </div>
            </div>
        </div>
    </div>
